...
discriminator.compile(loss=['binary_crossentropy','mse'], optimizer=RMSprop(lr=0.0002, decay=6e-8), metrics=['accuracy'])
...
adversarial.compile(loss=['binary_crossentropy', 'mse'], optimizer=RMSprop(lr=0.0001, decay=3e-8), metrics=['accuracy'])


inconNames = np.load('/DATA/CourseAI/datasets/icons/X.npy')
X = []
for i in range(0,len(inconNames)):
    img = cv2.imread('/DATA/CourseAI/datasets/icons/smallIcons/'+inconNames[i]+'.png')
    img = img.astype(np.float)/127.5 -1
    X.append(img[:,:,:3])

x_train = np.asarray(X)
x_labels = np.load('/DATA/CourseAI/datasets/icons/Y.npy')
